...
namespace Fm.DAL{
    ...
    /// food_series数据访问层类
    /// </summary>
	public partial class food_series
	{ 
		/// <summary>
		/// 增加一条数据
		/// </summary>
		public void Add(DBHelper myHelperMySQL ,Fm.Entity.food_series model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into food_series(");			
            strSql.Append("SeriesCode,SeriesName");
			...
			MySqlParameter[] parameters = {
			            new MySqlParameter("@SeriesCode", model.SeriesCode)  ,
                                    new MySqlParameter("@SeriesName", model.SeriesName)              
            };
		...
		/// 更新一条数据(所有字段)
		/// </summary>
		public int Update(DBHelper myHelperMySQL ,Fm.Entity.food_series model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update food_series set ");
		...
		public int Delete(DBHelper myHelperMySQL ,string strWhere, MySqlParameter[] parameters)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from food_series ");
        ...
        public int GetListNum(DBHelper myHelperMySQL, string strWhere, MySqlParameter[] parameters)
        {
			...
			strSql.Append("  FROM food_series  ");
            ...
            List<Fm.Entity.food_series> myList = new List<Fm.Entity.food_series>();
        ...
        /// </summary>
		public List<Fm.Entity.food_series> GetList(DBHelper myHelperMySQL ,int Top, string strWhere, string filedOrder, MySqlParameter[] parameters)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			strSql.Append(" a.SeriesCode, a.SeriesName  ");			
			strSql.Append("  FROM food_series a ");
			if (strWhere.Trim() != "")
            {
                strSql.Append(" WHERE " + strWhere);
            }
            ...
            using (MySqlDataReader dr = myHelperMySQL.ExecuteReader(strSql.ToString(), parameters))
            {
                while (dr.Read())
                {
                    Fm.Entity.food_series model = new Fm.Entity.food_series();
                    
                    															model.SeriesCode= dr["SeriesCode"].ToString();
																																								model.SeriesName= dr["SeriesName"].ToString();
        ...
        /// <param name="parameters">参数(若条件中未使用参数可为null).</param>
        /// </summary>
		public List<Fm.Entity.food_series> GetList(DBHelper myHelperMySQL ,int Top, string filedSelect, string strWhere, string filedOrder, MySqlParameter[] parameters)
		{
            ...
            using (MySqlDataReader dr = myHelperMySQL.ExecuteReader(strSql.ToString(), parameters))
            {
                while (dr.Read())
                {
                    	                ...
                    	                if (filedSelect.ToLower().Split(',').Where(x => x.Trim() == "a.seriescode").Count() > 0)
	                {
						                ...
						                if (filedSelect.ToLower().Split(',').Where(x => x.Trim() == "a.seriesname").Count() > 0)
	                {
															model.SeriesName= dr["SeriesName"].ToString();
																									} 
