...
namespace Fm.DAL{
	...
	{ 
		...
		/// 增加一条数据
		/// </summary>
		public void Add(DBHelper myHelperMySQL ,Fm.Entity.food_rel_type model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into food_rel_type(");			
            strSql.Append("FoodID,FoodType_Code,Sort,State,Createtime,Updatetime");
			strSql.Append(") values (");
            strSql.Append("@FoodID,@FoodType_Code,@Sort,@State,@Createtime,@Updatetime");            
            strSql.Append(") ");            
            	
			MySqlParameter[] parameters = {
			            new MySqlParameter("@FoodID", model.FoodID)  ,
		...
		/// 更新一条数据(所有字段)
		/// </summary>
		public int Update(DBHelper myHelperMySQL ,Fm.Entity.food_rel_type model,string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update food_rel_type set ");
		...
		public int Delete(DBHelper myHelperMySQL ,string strWhere, MySqlParameter[] parameters)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from food_rel_type ");
        ...
        public int GetListNum(DBHelper myHelperMySQL, string strWhere, MySqlParameter[] parameters)
        {
			...
			strSql.Append("  FROM food_rel_type  ");
        ...
        /// </summary>
		public List<Fm.Entity.food_rel_type> GetList(DBHelper myHelperMySQL ,int Top, string strWhere, string filedOrder, MySqlParameter[] parameters)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			strSql.Append(" a.FoodID, a.FoodType_Code, a.Sort, a.State, a.Createtime, a.Updatetime  ");			
			strSql.Append("  FROM food_rel_type a ");
			if (strWhere.Trim() != "")
            {
                strSql.Append(" WHERE " + strWhere);
            }
            ...
            using (MySqlDataReader dr = myHelperMySQL.ExecuteReader(strSql.ToString(), parameters))
            {
                while (dr.Read())
                {
                    Fm.Entity.food_rel_type model = new Fm.Entity.food_rel_type();
                    
                    															model.FoodID= dr["FoodID"].ToString();
        ...
        /// <param name="parameters">参数(若条件中未使用参数可为null).</param>
        /// </summary>
		public List<Fm.Entity.food_rel_type> GetList(DBHelper myHelperMySQL ,int Top, string filedSelect, string strWhere, string filedOrder, MySqlParameter[] parameters)
		{
            ...
            using (MySqlDataReader dr = myHelperMySQL.ExecuteReader(strSql.ToString(), parameters))
            {
                while (dr.Read())
                {
                    Fm.Entity.food_rel_type model = new Fm.Entity.food_rel_type();
                    	                ...
                    	                if (filedSelect.ToLower().Split(',').Where(x => x.Trim() == "a.foodid").Count() > 0)
	                {
															model.FoodID= dr["FoodID"].ToString();
																									} 
